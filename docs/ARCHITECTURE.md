...
- **Balancing objective (concrete):** choose trip distance per day so the **month-end closing balance lands in `[0, 8]` liters**. The running balance must **never go below 0** on any day. If the balance is trending too high (over-fueled relative to driving), route *more* distance / more stops to burn it down; if trending toward negative, reduce distance or convert days to zero-trip. Distribution model = **"whatever burns the right amount of fuel"**: the generator searches route/day combinations to hit `[0,8]`, using the §6b priority and tie-breakers only as soft guidance among otherwise-valid solutions.
...
- **Feasibility guard:** if no combination within the per-day caps (§7) can land the closing balance in `[0,8]` (e.g. far too much fuel purchased for the available working days), generation **fails with a clear message** rather than emitting an implausible sheet.
...
- **Balance never negative** on any row; **closing balance ∈ [0, 8]** liters.
...
- **Feasibility:** an over-fueled month that cannot reach `[0,8]` fails cleanly (tested with a crafted fixture).
...
Resolved: ✅ schema (§6a) · ✅ distances = undirected pairwise legs, summed · ✅ vehicle selection via `IsActive`, no within-month split · ✅ monthly only, yearly deferred · ✅ working days from Nager.Date + override, with hardening (§7a) · ✅ trip rules (§6b) · ✅ **balancing objective = closing balance in [0, 8] L, never negative** · ✅ **distribution = "whatever burns the right amount of fuel"** · ✅ **determinism NOT required; fuel events are the fixed anchors** · ✅ testing by invariants (§8).
