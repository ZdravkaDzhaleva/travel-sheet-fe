...
- **Done when:** the loading state renders skeleton rows (not plain text); a load failure shows a branded inline alert (`role="alert"`) **and** an error toast, plus a **Retry** button that re-invokes the master-data load. The T5.2 read-only assertion is relaxed from "zero buttons" to "no edit/input controls" (Retry and the outward link are non-mutating). Specs cover the skeleton state, the error+retry path, and the relaxed read-only assertion. Lint + build clean.
  - _Done: loading shows two shimmer skeleton cards (`role="status"` sr-only text + animated `.skeleton__bar`s, motion-reduced); failure renders a danger-bordered `role="alert"` with the message + a **Retry** button, and an `effect()` mirrors each distinct error to an error toast carrying a Retry action. Read-only test relaxed to "no input/textarea/select/contenteditable"; success view still 0 buttons, error view exactly 1 (Retry). Injects `ToastService`. 431 tests green, lint + build clean._
...
### [ ] T7.13 — Style consolidation sweep + restore tight style budget
...
- **Done when:** the remaining duplicated primitives are extracted to global/shared style partials under `src/styles/` (`_forms.scss` — `.form-field`/`.form-input`/`.form-label`; `_layout.scss` — `.page-head`/`.page-title`/`.page-sub`; `.pill`; `.kv` key/value rows), `@use`d from `styles.scss`; per-page copies removed and pages refactored onto them. Sign-in and Generate are folded onto the T7.8a `.card`/`.btn` primitives too. With duplication gone, lower `anyComponentStyle` `maximumError`/`maximumWarning` back to ~4 kB (or tighter) in `angular.json` so re-introducing a copied primitive trips the budget again. CONVENTIONS.md gains a short "shared primitives vs component styles" note (presentational → global class; behavioural → `shared/ui` component). All specs green; lint + build clean with the tightened budget.
